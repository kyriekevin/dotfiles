...
| `akinsho/bufferline.nvim` | 顶部 buffer tab 栏 | `event = "VeryLazy"` | `opts = function()` 让 `catppuccin.groups.integrations.bufferline` 在插件加载时才 require（跑在 catppuccin 之后）。`offsets` 让 neo-tree 侧边栏不覆盖 tab 栏。 |
...
| bufferline 颜色不对 / 看着像默认 | catppuccin 在 bufferline 之后加载（race） | 我们用 `opts = function() ... require("catppuccin.groups.integrations.bufferline").get() ... end` 保证 require 在插件 load 时执行（catppuccin 之后）。如果坏了，核对 `opts` 是 function 而不是 table。 |
...
- **bufferline 用 `opts = function()`。** `highlights = require("catppuccin.groups.integrations.bufferline").get()` 必须在 catppuccin 加载**之后**解析。table 形式（`opts = {...}`）会在读 spec 时求值，race。
