...
`chezmoi apply` 把 `dot_claude/settings.json` 渲染到 `~/.claude/settings.json`。hook 脚本不 vendor：Herdr 的 agent-state 脚本由 `herdr integration install claude` 在本机安装。`enabledPlugins` 只是声明插件意图，Claude 自己的 marketplace fetcher 在下次启动时会拉取。
...
| `hooks.SessionStart[*]` | `$HOME/.claude/hooks/herdr-agent-state.sh session` | 把 Claude session 身份/状态报告给 Herdr。脚本是本机产物，由 `herdr integration install claude` 安装。 |
...
| `dot_claude/hooks/` | Herdr 生成的 agent-state hook 脚本 | 本机 integration 安装产物。settings 只 pin 调用方式，不 vendor 生成脚本。 |
...
Claude Code 支持 lifecycle events × handler 类型（`command` / `http` / `prompt` / `agent`）。user 级 hooks 我们用两个：

- **PreToolUse, matcher=Bash** → `npx block-no-verify@1.1.2`。每次 Bash 工具调用前触发；非零退出码告诉 Claude 命令被拦截。
- **SessionStart, matcher=\*** → Herdr 本机 `herdr-agent-state.sh`。让 Herdr 能把 Claude session 和 pane 关联起来，用于 agent 状态/恢复。
